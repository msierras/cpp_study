...
class Set{
    private:
        ...
        /* The find() function is private because it is not used outside of the Set class. */
...
bool Set::isMember(char ch){
    if(find(ch) != -1) return true; // using previous class member function find in isMember
    return false; 
}
...
Set Set::operator+(char ch){
    ...
    // see if element already exists, if not found, then add 
    if(find(ch) == -1){ 
...
Set Set::operator-(char ch){
    Set newset;
    int i = find(ch);   /* if i = -1 then j != i is always true and all members[j] are added. Only if
                            the ch exists then do we delete it! */
...
/* Union and Difference use the previous two operators! */

// Set union
Set Set::operator+(Set ob2){
    Set newset = *this; // copy the first set, i.e, ob1 from ob1 + ob2 call

    // Add unique elements from second set.
    for(int i=0; i < ob2.len; i++) newset = newset + ob2.members[i];    // using the previous + operator!
...
Set Set::operator-(Set ob2){
    ...
    // Subtract the elements from second set.
    for(int i = 0; i < ob2.len; i++ ) newset = newset - ob2.members[i]; // using the previous - operator!
...
int main(){
    ...
    cout << "Testing for membership of 'B' and 'T' in s1 using isMember().\n";
